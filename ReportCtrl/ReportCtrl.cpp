...
#include <afxtempl.h>
/*#include "../XScanner/XScannerDlg.h"*/
#ifdef _DEBUG
...
void CReportCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if ( CDDS_ITEMPREPAINT == lplvcd->nmcd.dwDrawStage )
	{
		...
		if( CListCtrl::GetItemState(lplvcd->nmcd.dwItemSpec,CDIS_SELECTED)) 
		{
			//pLVCD->nmcd.uItemState ^= CDIS_SELECTED;// 冷뚤!
			lplvcd->nmcd.uItemState &= ~CDIS_SELECTED; //寧湳角뚤돨
...
void CReportCtrl::OnDropFiles(HDROP hDropInfo)
{
...
// 		CXScanList *pMain=(CXScanList *)AfxGetApp()->m_pMainWnd;
// 		if(this->GetDlgCtrlID() == IDC_LIST1)
// 		{
// 			pMain->OpenFile((LPCTSTR)szFilePathName);
// 		}
