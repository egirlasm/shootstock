		...
		for (int nIdx = 0; nIdx < nFieldCnt; nIdx++)
		{
			...
			if(nIdx== 0){
				if(strData == m_boardJongmokCode){
					...
					strData = theApp.m_khOpenApi.GetCommData(sTrcode, strRQName, 0, L"250ÃÖ°í");	strData.Trim();
					m_staticHigh = strData;
					m_staticHigh = L"50ÁÖÃÖ°í: "+m_staticHigh.Mid(1,m_staticHigh.GetLength() -1);
					strData = theApp.m_khOpenApi.GetCommData(sTrcode, strRQName, 0, L"250ÃÖÀú");	strData.Trim();
					m_staticLow = strData;
					m_staticLow = L"50ÁÖÃÖÀú: "+m_staticLow.Mid(1,m_staticLow.GetLength() -1);
					strData = theApp.m_khOpenApi.GetCommData(sTrcode, strRQName, 0, L"¸ÅÃâ¾×");	strData.Trim();
					m_staticSales = L"¸ÅÃâ¾×: "+strData;
					strData = theApp.m_khOpenApi.GetCommData(sTrcode, strRQName, 0, L"¿µ¾÷ÀÌÀÍ");	strData.Trim();
					m_staticProfit = L"¿µ¾÷ÀÌÀÍ: "+strData;
...
void CshootStockDlg::OnReceiveMsgKhopenapictrl(LPCTSTR sScrNo, LPCTSTR sRQName, LPCTSTR sTrCode, LPCTSTR sMsg)
{
	...
	if (m_mapScreen.Lookup(strKey, (void *&)pWnd) && pWnd)
	{
		...
		if(strErrMsg.Find(_T("ºñ¹Ð¹øÈ£")) != -1 && strErrMsg.Find(_T("¿À·ù")) != -1){
			::MessageBox(m_hWnd,_T("ºñ¹Ð¹øÈ£ ¿À·ù! 5¹ø ¿À·ù½Ã Å«ÀÏ³²,Á¦´ë·Î ÀÔ·ÂÇÏÀÚ!"),_T("ºñ¹Ð¹øÈ£ ¿À·ù!"),MB_ICONERROR);
		...
		if(!theApp.g_is_password_success && strErrMsg.Find(_T("Á¶È¸")) != -1 && strErrMsg.Find(_T("¿Ï·á")) != -1){
		...
		case 0:		// ÇöÀç°¡
			{
				...
				//+		sMsg	0x08dabe0c "[850188] ºñ¹Ð¹øÈ£ 2È¸ ¿À·ù, 5È¸ ¿À·ù½Ã ºñ¹Ð¹øÈ£ Àçµî·Ï(¹®ÀÇ1544-9000)"	const wchar_t *
				TraceOutputW(sMsg);
			}
			break;
...
void CshootStockDlg::MainOnReceiveRealDataKhopenapictrl(LPCTSTR sJongmokCode, LPCTSTR sRealType, LPCTSTR sRealData)
{
	...
	if (strRealType == _T("Àå½ÃÀÛ½Ã°£"))			// °ü½ÉÁ¾¸ñÁ¤º¸ ¼³Á¤
	{
		...
		case 2:
			...
			TraceOutputW(fmt);
			break;
		case 3: //Àå½ÃÀÛÀÌ µÇ¿´½À´Ï´Ù
			fmt.Format(L"ÇöÀç½Ã°£ %s:%s:%s Àå½ÃÀÛÀÌ µÇ¿´½À´Ï´Ù ",szHour1,szMinute1,szSecond1,szMinute);
			TraceOutputW(fmt);
			isStockMarketOpen = true;
			break;
		case 4:
			fmt.Format(L"ÇöÀç½Ã°£ %s:%s:%s ÀåÁ¾·á µÇ¿´½À´Ï´Ù ",szHour1,szMinute1,szSecond1,szMinute);
			TraceOutputW(fmt);
			break;
		case 8:
			...
			if(strData == L"2"){
				...
				if(m_checkedSubject.Lookup(sJongmokCode,(CObject*&)subject)){
					...
					if(subject->is_checked_all()){ //¸Å¼öÁÖ¹®ÀÌ ÀüºÎ Ã¼°áÀÌ µÈÈÄ¿¡ ¸ÅµµÁÖ¹®À» °Ç´Ù ¾Æ´Ï¸é ÀÚ²ÙÀÌ»óÇÏ°Ô ºÎºÐ¸¸ ÁöÁ¤¸Åµµ°¡ µÊ
						...
						fmt.Format(L"¼³Á¤µÈ ¸Åµµ ÆÛ¼¾Æ® %0.2f%% Áï ¸Åµµ°¡°Ý = %d",m_sellPercentage,nPrice);
						TraceOutputW(fmt);
...
void CshootStockDlg::OnEventConnect(LONG nItemCnt)
{
	if (nItemCnt == 0)
	{
		...
		//ºñ¹Ð¹øÈ£ = »ç¿ë¾ÈÇÔ(°ø¹é)
			theApp.m_khOpenApi.SetInputValue(L"ºñ¹Ð¹øÈ£"	, L"2419");

		//ºñ¹Ð¹øÈ£ÀÔ·Â¸ÅÃ¼±¸ºÐ = 00
			theApp.m_khOpenApi.SetInputValue(L"ºñ¹Ð¹øÈ£ÀÔ·Â¸ÅÃ¼±¸ºÐ"	,  L"00");

		/*Á¶È¸±¸ºÐ = 1:ÇÕ»ê, 2:°³º°*/
			theApp.m_khOpenApi.SetInputValue(L"Á¶È¸±¸ºÐ"	,  L"1");
...
void CshootStockDlg::InitTabControl(void)
{
	...
	m_TabControl.InsertItem(5, _T("ÀÚµ¿¸Å¸Å"));
	pImageList.Detach();
		...
		if( m_dealList.GetItemState(i, LVIS_SELECTED) == LVIS_SELECTED )
		{
			...
			if(m_checkedSubject.Lookup(strJongCode,(CObject*&)subject) ){
				if (m_mapOrderNo.Lookup(strJongCode, strOrderNo))
				{
					...
					//°É¾î³õÀº ¹ÌÃ¼°áÀÌ ¾øÀ¸´Ï ¹Ù·Î ¸Åµµ ÇÔ
					_Trace(L"°É¾î³õÀº ÁÖ½ÄÀÌ ¾ø¾î ¹Ù·Î ¸ÅµµÇÔ///°³¹ß¹Ì¿Ï¼º/¸ð´ÏÅÍ¸µ Á¤Áö °í¹Î");
				}
