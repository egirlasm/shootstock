		...
		if(isRunning){ //¼ÕÀý ÆÇ´Ü 
			...
			if(pMain->m_checkedCode == sJongmokCode){
				...
				if(nPrice < m_boughtPrice){
					...
					if(nPercentage > 0.03){ // 3%ÀÌ»óÀÌ¸é ¼ÕÀý
...
void CshootStockDlg::MainOnReceiveChejanData(LPCTSTR sGubun, LONG nItemCnt, LPCTSTR sFidList){
	...
	if(strGuBun == L"0"){//Ã¼°áÅëº¸
		...
		if(orderStatus == L"Á¢¼ö"){
			...
			if(strData == L"2"){
				isRunning = true;
				int nPrice = _wtoi(strPrice); //ÃÖÁ¾ Ã¼°áµÈ ¸Å¼ö°¡·Î ÁöÁ¤¸Åµµ °É¾î³öµµ ±¦ÂúÀ»µí
				m_boughtPrice = nPrice;
				m_checkedCount += _wtoi(strCount); //Ã¼°áµÈ ¼ö·® ¸Å¹ø ´õÇØÁÖ±â  Àý¹Ý¾¿ Ã¼°áµÉ‹š°¡ ÀÖÀ¸´Ï±ñ
				if(m_checkedCount == m_boughtCount){ //¸Å¼öÁÖ¹®ÀÌ ÀüºÎ Ã¼°áÀÌ µÈÈÄ¿¡ ¸ÅµµÁÖ¹®À» °Ç´Ù ¾Æ´Ï¸é ÀÚ²ÙÀÌ»óÇÏ°Ô ºÎºÐ¸¸ ÁöÁ¤¸Åµµ°¡ µÊ
